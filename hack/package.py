...
parser.add_argument('version', type=str, nargs='?',
                    metavar='version', help='version to package')
...
baseolm = os.path.join(script_home, "..", "deploy", "olm-catalog")
config = os.path.join(script_home, "..", "config")
releases = os.path.join(script_home, "..", "releases", args.version)
olm = os.path.join(script_home, "..", "deploy", "olm-catalog", args.version)
latest = os.path.join(script_home, "..", "releases", "latest")

# load defaults
with open(os.path.join(baseolm, "templates", "defaults.yaml"), 'r') as stream:
...
if os.path.exists(latest):
    os.unlink(latest)
os.symlink(os.path.join("..", "releases", args.version),
           os.path.join("..", "releases", "latest"))
...
for filename in os.listdir(crds):
    shutil.copyfile(os.path.join(crds, filename),
                    os.path.join(releases, filename))

# copy serviceaccount
sa_file = "serviceaccount.yaml"
shutil.copyfile(os.path.join(config, "rbac", sa_file),
                os.path.join(releases, sa_file))

# copy rbac_role
rbac_role_file = "rbac_role.yaml"
# load rbac_role from kubebuilder
shutil.copyfile(os.path.join(config, "rbac", rbac_role_file),
                os.path.join(releases, rbac_role_file))

# copy role binding
rb = "rbac_role_binding.yaml"
shutil.copyfile(os.path.join(config, "rbac", rb),
                os.path.join(releases, rb))

# copy deployment file
deploy_file = "deployment.yaml"
# load rbac_role from kubebuilder
with open(os.path.join(config, "manager", deploy_file), 'r') as deploystream:
    deploy = yaml.safe_load(deploystream)
# update image
deploy['spec']['template']['spec']['containers'][0]['image'] = defs['image'] + \
    ":"+args.version[1:]
# write it back with new name
with open(os.path.join(releases, deploy_file), "w") as outfile:
...
for filename in os.listdir(releases):
    ...
    if (filename.find("v1alpha1") < 0):
        continue
    shutil.copyfile(os.path.join(releases, filename),
                    os.path.join(olm, rename_crd(filename)))

# copy package file
with open(os.path.join(baseolm, "templates", "template.package.yaml"), 'r') as stream:
    ...
    with open(os.path.join(olm, defs['operator_name']+"."+"package.yaml"), "w") as outfile:
        yaml.dump(pkg, outfile, default_flow_style=False)

# fill in cluster service version from template, deployment and roles
with open(os.path.join(baseolm, "templates", "template.clusterserviceversion.yaml"), 'r') as stream:
    ...
    csv['spec']['description'] = defs['description']
    ...
    for i in range(len(defs['crd'])):
        ...
        if crd != None:
            ...
            # add examples
            example_file_name = os.path.join(baseolm, "..", 'crds', "ibmcloud_" + crd_version + '_' + kind.lower() + '_cr.yaml')
            try:
                with open(example_file_name) as example_file:
                    ex = yaml.safe_load(example_file)
    ...
    csv['metadata']['annotations']['alm-examples'] = literal(
        json.dumps(alm_examples))

    with open(os.path.join(olm, defs['operator_name']+"."+args.version+".clusterserviceversion.yaml"), "w") as outfile:
