	...
	if strings.HasPrefix(url, "http") {
		...
		if err != nil {
			...
			if rc == 400 || rc == 401 || rc == 403 {
				token, err := GetSecretValue(ctx, "seed-secret", "access_token", true)
				if err != nil {
					return nil, err
				}
				...
				value, e2 := GetSecretValue(ctx, secretname, "token", true)
				if e2 != nil {
					// Try global token
					value, e2 = GetSecretValue(ctx, parts[0], "token", true)
					if e2 != nil {
						return nil, e2
					}
				}
